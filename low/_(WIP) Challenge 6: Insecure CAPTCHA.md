...
Protip - this module requires keys for Captcha v2. V3 will not work.
...
From the basic form, we try to change our password to "test":
...
Hm, we see three pages, but not any hint of how they pass data to each other. 

Let's try using a tool called Burp Suite to examine the requests as they go back and forth. Let's go to the "Proxy" module/tab, then the "Intercept" tab, making sure the button "Intercept is on" is enabled. If it isn't, click the "Intercept is off" button to change it. Let's then try running through the same execution flow, but watching the requests we send.

<i>Note: You may need to temporarily disable interception until after you verify the Captcha to make life a little easier.</i>
...
It works! We were able to bypass the Captcha completely by jsut replaying the second, confirmation HTTP request. In the real world, we would try to steal the "admin" cookie and try to visit the malicious URL, or we could trick the end user into clicking the malicious link itself. Regardless, for our intents and purposes, the challenge is complete!
